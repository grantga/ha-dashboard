...
export default function useMediaPlayer(entityId: EntityName): UseMediaPlayer {
    ...
    const [volume, setVolumeState] = useState<number | null>(() => (entity && entity.attributes && typeof entity.attributes.volume_level === 'number') ? entity.attributes.volume_level : null);
    const [muted, setMutedState] = useState<boolean | null>(() => (entity && entity.attributes && typeof entity.attributes.is_volume_muted === 'boolean') ? entity.attributes.is_volume_muted : null);
    ...
    const [currentSource, setCurrentSource] = useState<string | null>(() => (entity && entity.attributes && (entity.attributes.source || entity.attributes.input_source || entity.attributes.input)) ?? null);
    ...
    const setSource = useCallback(async (source: string) => {
        try {
